...
function extractNameFromTemplate(value) {
   let n = value.indexOf("John");
   if( n !== -1){
     return value.substr(n,8);
   }else if(value.indexOf("Chuck") !== -1){
     return value.substr(value.indexOf("Chuck"),12);
   }
}
...
function removeFirstOccurrences(str, value) {
    ...
    let firststr = str.slice(0,index);
    let secondstr = str.substr(index+value.length);
...
function getRectangleString(width, height) {
   let upperAngle = '┌' + '─'.repeat(width - 2) + '┐\n';
   let lines = '│' + ' '.repeat(width - 2) + '│\n';
   let lowerAngle = '└' + '─'.repeat(width - 2) + '┘\n';
   return upperAngle + lines.repeat(height - 2) + lowerAngle;
}
...
function encodeToRot13(str) {
    let strCode = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
    let strDecode = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
    let newstr = [];
    let j = 0;
    for(let i = 0; i<str.length;i++){
      if(strCode.indexOf(str[i]) !== -1 ){
        newstr[j] = strDecode[strCode.indexOf(str[i])];
        j++
      } else {
        newstr[j] = str[i];
        j++;
